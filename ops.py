...
def sndownconv(x, output_dim, kwidth=5, pool=2, init=None, uniform=False,
             bias_init=None, is_ref=False, sn_iters=1, name='downconv'):
    """ Downsampled convolution 1d """
    x2d = tf.expand_dims(x, 2)
    w_init = init
    if w_init is None:
    ...
    with tf.variable_scope(name):
        W = tf.get_variable('W', [kwidth, 1, x.get_shape()[-1], output_dim],
                            initializer=w_init)
        W = spectral_normed_weight(W, num_iters=sn_iters,update_collection=None)
...
def sndeconv(x, output_shape, kwidth=5, dilation=2, init=None, uniform=False,
           bias_init=None, is_ref=False, sn_iters=1, name='deconv1d'):
    input_shape = x.get_shape()
    in_channels = input_shape[-1]
...
def average_gradients(tower_grads):
    ...
    for grad_and_vars in zip(*tower_grads):
        ...
        #grad = tf.concat(grads, 0) ############### feedback
